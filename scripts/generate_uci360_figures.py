    ...
    return [
        {
            ...
            "caption": (
                ...
                "**This is the pre-specified draw, and on this figure's own quantity it is the "
                "most extreme of the ten.** The mean–median gap at $N=2$ is $+0.997$ here "
                "against $+0.105$ to $-0.014$ in the other nine draws, where the two curves "
                "nearly coincide; the median at $N=2$ is stable at 0.964–1.094 throughout but "
                "the mean is 2.005 here and 0.979–1.149 elsewhere. The divergence shown is "
                "therefore what a realised tail looks like, not what a laboratory should "
                "expect. Figure 5 gives the distribution across draws. "
            ...
            "limitations": (
                "One panel draw, and the one whose tail was realised: the mean–median gap here "
                "is ten times any other draw's. Cells share source-model fits and target "
                "windows and are not independent replicates, so the spread is descriptive "
                "rather than a confidence statement. "
                + shared_panel
            ),
        ...
        },
        {
            ...
            "caption": (
                ...
                "**Both panels are the pre-specified draw, and it is the most extreme of the ten "
                "on both quantities shown**: its worst ratio is 44.5 against a next-worst of 4.7, "
                "and it is the only draw whose count in (b) reaches zero at four and stays there "
                "(the ten first-clean budgets are 4, 4, 4, 4, 4, 5, 6, 8, 20, 20). The budget at "
                "which the count vanishes is therefore not a floor; Figure 5 gives the "
                "distribution and Table 6 the pooled counts."
            ),
            "claim": (
        ...
        },
        {
            ...
            "caption": (
                ...
                "**This is the pre-specified draw, the only one of ten in which the inversion "
                "band is populated at all**; in the other nine the minimum slope at two "
                "references runs $+0.275$ to $+0.408$, so the band is empty. No inversion "
            ...
            "limitations": (
                "One panel draw, and the only one of ten with any inverted slope, so the "
                "inversion band should not be read as commonly occupied. The calibrator is "
                "fitted on the reference panel while this slope is evaluated on the held-out "
                "set, so the two are linked only through the panel draw. The standard deviation is not monotone in "
        ...
        },
        {
            ...
            "caption": (
                ...
                "$N=2$ does not order cells by safety. **Both panels are the pre-specified draw, "
                "and it is the only one of ten in which an inversion occurred**; in the other "
                "nine the top diagnostic quartile at $N=2$ is no worse than the rest, so the "
                "reversal in (b) is an existence proof rather than a general pattern. The pooled "
                "coefficient in (a) is also mostly a contrast between analytes rather than an "
                "ordering within one: ranking within analyte first reduces it from 0.814 to "
                "0.192. Note the symmetric logarithmic vertical axis in (b), linear within "
                "$\\pm0.1$, and the different vertical ranges of the two panels. "
                "A gate keyed on a high diagnostic can therefore recommend recalibration exactly "
